...
class Metaboxes {
	...
	public function images_tab( $post ) {
		...
		echo wp_kses( apply_filters( 'envira_gallery_images_tab_notice', '', $post ), $this->wp_kses_allowed_html );
		...
		<ul id="envira-gallery-output" class="envira-gallery-images-output <?php echo esc_attr( $layout ); ?>" data-view="<?php echo esc_attr( $layout ); ?>" data-sortable="<?php echo esc_html( $is_sortable ); ?>">
			...
			if ( ! empty( $gallery_data['gallery'] ) ) {
				foreach ( $gallery_data['gallery'] as $id => $data ) {
					echo wp_kses( $this->get_gallery_item( $id, $data, $post->ID ), $this->wp_kses_allowed_html );
				}
			}
			?>
		</ul>
					...
					<tbody>
						...
						<tr id="envira-config-lightbox-toolbar-box">
								...
								<td>
									...
									<span class="description"><?php esc_html_e( 'Enables or disables the gallery lightbox toolbar.', 'envira-gallery' ); ?></span>
								</td>
							</tr>
						...
						<tr id="envira-config-lightbox-aspect-box">
								<th scope="row">
									<label for="envira-config-lightbox-aspect"><?php esc_html_e( 'Keep Aspect Ratio?', 'envira-gallery' ); ?></label>
								</th>
								<td>
									<input id="envira-config-lightbox-toolbar" type="checkbox" name="_envira_gallery[aspect]" value="<?php echo esc_html( envira_get_config( 'aspect', $gallery_data ) ); ?>" <?php checked( envira_get_config( 'aspect', $gallery_data ), 1 ); ?> />
									<span class="description"><?php esc_html_e( 'If enabled, images will always resize based on the original aspect ratio.', 'envira-gallery' ); ?></span>
								</td>
							</tr>
			...
			<table class="form-table">
				<tbody>
					...
					<tr id="envira-config-classes-box">
						...
						<td>
							<textarea id="envira-config-classes" rows="5" cols="75" name="_envira_gallery[classes]" placeholder="<?php esc_html_e( 'Enter custom gallery CSS classes here, one per line.', 'envira-gallery' ); ?>"><?php echo esc_html( implode( "\n", envira_get_config( 'classes', $gallery_data ) ) ); ?></textarea>
	...
	public function save_meta_boxes( $post_id, $post ) {
		...
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			...
			if ( isset( $_POST['_inline_edit'] ) ) {
				...
				// Update Settings.
				$settings['config']['columns']                 = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['columns'] ) ) );
				$settings['config']['gallery_theme']           = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['gallery_theme'] ) ) );
				$settings['config']['justified_gallery_theme'] = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['justified_gallery_theme'] ) ) );
		...
		$new_lb_theme = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['lightbox_theme'] ) ) );
		...
		$settings['config']['justified_last_row'] = sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['justified_last_row'] ) );
		...
		$settings['config']['gutter']             = absint( $_POST['_envira_gallery']['gutter'] );
		$settings['config']['margin']             = absint( $_POST['_envira_gallery']['margin'] );
		$settings['config']['image_size']         = sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['image_size'] ) );
		$settings['config']['crop_width']         = absint( $_POST['_envira_gallery']['crop_width'] ) > 0 ? absint( $_POST['_envira_gallery']['crop_width'] ) : envira_get_config_default( 'crop_width' );
		$settings['config']['crop_height']        = absint( $_POST['_envira_gallery']['crop_height'] ) > 0 ? absint( $_POST['_envira_gallery']['crop_height'] ) : envira_get_config_default( 'crop_height' );
		$settings['config']['crop']               = isset( $_POST['_envira_gallery']['crop'] ) ? 1 : 0;
		$settings['config']['crop_position']      = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['crop_position'] ) ) );
		...
		$settings['config']['description_position'] = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['description_position'] ) ) );
		$settings['config']['description']          = trim( wp_kses( wp_unslash( $_POST['_envira_gallery']['description'] ), $this->wp_kses_allowed_html ) );
		...
		$settings['config']['image_sizes_random']   = isset( $_POST['_envira_gallery']['image_sizes_random'] ) ? stripslashes_deep( sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['image_sizes_random'] ) ) ) : array();
		...
		$settings['config']['lightbox_theme']         = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['lightbox_theme'] ) ) );
		$settings['config']['lightbox_image_size']    = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['lightbox_image_size'] ) ) );
		$settings['config']['title_display']          = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['title_display'] ) ) );
		$settings['config']['lightbox_title_caption'] = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['lightbox_title_caption'] ) ) );

		$settings['config']['arrows']                     = isset( $_POST['_envira_gallery']['arrows'] ) ? 1 : 0;
		$settings['config']['arrows_position']            = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['arrows_position'] ) ) );
		...
		$settings['config']['toolbar_position']           = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['toolbar_position'] ) ) );
		$settings['config']['loop']                       = isset( $_POST['_envira_gallery']['loop'] ) ? 1 : 0;
		$settings['config']['lightbox_open_close_effect'] = preg_replace( '#[^A-Za-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['lightbox_open_close_effect'] ) ) );
		$settings['config']['effect']                     = preg_replace( '#[^A-Za-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['effect'] ) ) );
		...
		$settings['config']['thumbnails_position']    = preg_replace( '#[^a-z0-9-_]#', '', sanitize_text_field( wp_unslash( $_POST['_envira_gallery']['thumbnails_position'] ) ) );
