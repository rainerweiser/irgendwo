...
class Tools {
							...
							<form id="envira-fix-migration" method="post">
								...
								<?php if ( ! empty( $galleries ) ) { ?>
									...
									foreach ( $galleries as $gallery ) {
										...
										?>
										<option value="<?php echo esc_html( $gallery['id'] ); ?>"><?php echo esc_html( $post->post_title ); ?> -- <?php echo count( $gallery['gallery'] ); ?> <?php esc_html_e( 'Items (ID: ' ); ?><?php esc_attr_e( $gallery['id'] ); ?>)</option>
									<?php } ?>
								</select>
							<?php } ?>
	...
	public function fixed_migration() {
		...
		<div class="notice updated below-h2">
			<p><strong><?php echo esc_html( $fixed_galleries ); ?> <?php echo_html_e( ' galleries(s) fixed successfully.', 'envira-gallery' ); ?></strong></p>
		</div>
		<?php
