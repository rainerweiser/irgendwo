...
class Widget extends \ WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		$gallery_id = $title = $number_of_images = $gallery_args = false;
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number_of_images' ); ?>">
				<?php _e( 'Number Of Images To Display?', 'envira-gallery' ); ?>
			</label>
			<input class="envira_widget_number_of_images" value="<?php echo $number_of_images; ?>" type="text" maxlength="3" name="<?php echo esc_attr( $this->get_field_name( 'number_of_images' ) ); ?>" id="<?php echo esc_attr( $this->get_field_name( 'number_of_images' ) ); ?>" />
...
class EnviraWidgetRandom extends \ WP_Widget {
	...
	public function widget_custom_data( $data, $atts, $post ) {
		...
		if ( empty( $atts['transients'] ) ) {
			return;
		}

		$atts['transients'] = urldecode( $atts['transients'] );

		$transient = get_transient( '_eg_gallery_data_' . $atts['id'] );

		$data = json_decode( $transient, true );

		$data['gallery_images_raw'] = get_transient( '_eg_gallery_images_raw_' . $atts['id'] );
	...
	public function widget( $args, $instance ) {
		...
		$gallery_id = $title = $envira_display_images_lightbox = $gallery_id_for_image = false;
		...
		if ( $gallery_id ) {
			...
			$image_ids = get_transient( '_eg_gallery_widget_random_image_ids' );
			...
			if ( $image_ids === false ) {
				...
				if ( ! empty( $query->posts ) ) {
					...
					foreach ( $query->posts as $image_id ) {
						...
						if ( $galleries !== false && is_array( $galleries ) ) {
							foreach ( $galleries as $the_gallery_id ) {
								...
								// echo '<br>gallery id: ' .$the_gallery_id;
								// echo '<br>image_id: ' . $image_id;
								if ( $found === false && get_post_type( $the_gallery_id ) === 'envira' ) {
									$images_in_gallery = get_post_meta( $the_gallery_id, '_eg_in_gallery', true );
									if ( in_array( $image_id, $images_in_gallery ) ) {
										// echo '<br>image in gallery:';
										// print_r ($images_in_gallery);
										// echo '<br>';
										$found                = true;
			...
			if ( ! empty( $image_ids ) ) {
				...
				$args                             = array();
				$args['transients']               = json_encode( $transients );
				$args['dynamic']                  = 'true';
				$args['type']                     = 'widget';

				$transient_time_duration = apply_filters( 'envira_gallery_widget_random_image_duration', DAY_IN_SECONDS );

				$transient = set_transient( '_eg_gallery_images_raw_' . $gallery_id . '_widget_random_images', json_encode( $gallery_images_raw ), $transient_time_duration );
				$transient = set_transient( '_eg_gallery_data_' . $gallery_id . '_widget_random_images', json_encode( $data ), $transient_time_duration );
	...
	public function form( $instance ) {
		...
		<?php do_action( 'envira_gallery_widget_middle_form', $instance ); ?>
		<p>
			<label for="<?php echo $this->get_field_id( 'number_of_images' ); ?>"><?php _e( 'Number Of Images To Display?', 'envira-gallery' ); ?></label>
			<input class="envira_widget_random_number_of_images" min="1" max="50" value="<?php echo $number_of_images; ?>" type="number" name="<?php echo esc_attr( $this->get_field_name( 'number_of_images' ) ); ?>" id="<?php echo esc_attr( $this->get_field_name( 'number_of_images' ) ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'envira_display_images_lightbox' ); ?>">
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'gallery_id' ); ?>"><?php _e( 'Use Settings From This Gallery:', 'envira-gallery' ); ?></label>
