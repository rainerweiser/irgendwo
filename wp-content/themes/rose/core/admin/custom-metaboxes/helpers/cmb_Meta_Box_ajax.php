...
class cmb_Meta_Box_ajax {
	...
	public function oembed_handler() {
		...
		if ( empty( $oembed_string ) )
			self::send_result( '<p class="ui-state-error-text">'. esc_html__( 'Please Try Again', 'rose' ) .'</p>', false );
	...
	public static function get_oembed( $url, $object_id, $args = array() ) {
		...
		if ( $check_embed && $check_embed != $fallback )
			return '<div class="embed_status">'. $check_embed .'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button" rel="'. $args['field_id'] .'">'. esc_html__( 'Remove Embed', 'rose' ) .'</a></p></div>';

		// Otherwise, send back error info that no oEmbeds were found
		return '<p class="ui-state-error-text">'. sprintf( esc_html__( 'No oEmbed Results Found for %s. View more info at', 'rose' ), $fallback ) .' <a href="http://codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>.</p>';
