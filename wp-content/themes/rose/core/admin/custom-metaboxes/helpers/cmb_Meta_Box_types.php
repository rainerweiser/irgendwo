...
class cmb_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button" href="#"><?php esc_html_e( 'Add Row', 'rose' ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo $class; ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php esc_html_e( 'Remove', 'rose' ); ?></a>
			</td>
		</tr>
...
//		return  $this->field->args('parent_name') .  $name . ( $this->field->args( 'repeatable' ) ? '['. $this->iterator .']' : '' );
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. esc_html__( 'No terms', 'rose' ) .'</label></li>';
		} else {
			foreach ( $terms as $term ) {
				$args = array(
					'value' => $term->slug,
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => esc_html__( 'Add or Upload File', 'rose' ),
			'name'  => '', 'id'  => '',
		) );
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove Image', 'rose' ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						esc_html__( 'File:', 'rose' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. esc_html__( 'Download', 'rose' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove', 'rose' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
	...
	public function file() {
		...
		'<input class="cmb_upload_button button" type="button" value="'. esc_html__( 'Add or Upload File', 'rose' ) .'" />',
		$this->_desc( true );
		...
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_media_status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo '<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. esc_html__( 'Remove Image', 'rose' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo esc_html__( 'File:', 'rose' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value, '" target="_blank" rel="external">'. esc_html__( 'Download', 'rose' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. esc_html__( 'Remove', 'rose' ) .'</a>)';
				}
			}
		echo '</div>';
	}
